...
public class oldKernel extends Thread
{
  ...
  private oldControlPanel controlPanel ;
  ...
  private final Set<Integer> physMapped = new HashSet<>();
  ...
  public void init( String commands , String config )  
  {
    ...
    long low = 0;
    long addr = 0;
    long address_limit = (block * maxVirtPageIndex +1)-1;
      ...
      try 
      {
        ...
        while ((line = in.readLine()) != null) 
        {
          ...
          { 
            ...
            {
              tmp = st.nextToken();
              maxVirtPageIndex = Common.s2i(st.nextToken()) - 1;
              if ( maxVirtPageIndex < 2 || maxVirtPageIndex > 63 )
              {
                System.out.println("MemoryManagement: numpages out of bounds.");
                System.exit(-1);
              }
              address_limit = (block * maxVirtPageIndex +1)-1;
            }
          }
        }
        in.close();
      } catch (IOException e) { /* Handle exceptions */ }
      for (i = 0; i <= maxVirtPageIndex; i++)
      {
        high = (block * (i + 1))-1;
        low = block * i;
...

        {
          ...
          { 
            ...
            { 
              ...
              if ((0 > id || id > maxVirtPageIndex) || (-1 > physical || physical > ((maxVirtPageIndex - 1) / 2)))
              {
    ...
    for (i = 0; i <= maxVirtPageIndex; i++)
    {
      ...
      if ( page.physical != -1 )
      {
        physMapped.add(page.physical);
        map_count++;
      }
      for (j = 0; j <= maxVirtPageIndex; j++)
      {
    ...
    if (map_count <= (maxVirtPageIndex - 1) / 2) {
      for (i = 0; i <= maxVirtPageIndex; i++) {
        Page page = (Page) memVector.elementAt(i);
        if (page.physical == -1 && map_count <= (maxVirtPageIndex - 1) / 2) {
          for (int k = 0; k <= (maxVirtPageIndex - 1) / 2; ++k) {
            if (!physMapped.contains(k)) {
              page.physical = k;
              physMapped.add(k);
  ...
  public void setControlPanel(oldControlPanel newControlPanel)
  {
  ...
  public void step()
  {
    ...
    getPage( Virtual2Physical.pageNum( instruct.addr , maxVirtPageIndex, block ) );
    ...
    {
      Page page = ( Page ) memVector.elementAt( Virtual2Physical.pageNum( instruct.addr , maxVirtPageIndex, block ) );
      if ( page.physical == -1 ) 
      {
        if ( doFileLog )
        {
        ...
        oldPageFault.replacePage( memVector , maxVirtPageIndex, Virtual2Physical.pageNum( instruct.addr , maxVirtPageIndex, block ) , controlPanel );
    ...
    if ( instruct.inst.startsWith( "WRITE" ) ) 
    {
      ...
      if ( page.physical == -1 ) 
      {
        ...
        oldPageFault.replacePage( memVector , maxVirtPageIndex, Virtual2Physical.pageNum( instruct.addr , maxVirtPageIndex, block ) , controlPanel );          controlPanel.pageFaultValueLabel.setText( "YES" );
      } 
      else 
      {
        page.M = 1;
    ...
    {
      Page page = ( Page ) memVector.elementAt( i );
      if ( page.R == 1 && page.lastTouchTime == 1 )
      {
        page.R = 0;
      }
      if ( page.physical != -1 ) 
      {
        page.inMemTime = page.inMemTime + 1;
        page.lastTouchTime = page.lastTouchTime + 1;
      }
    }
    runs++;
    controlPanel.timeValueLabel.setText( ( runs ) + " (ns)" );
  }
